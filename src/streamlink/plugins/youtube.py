...
class Solver(ABC):
    ...
    def validate_output(output: str | dict, request: NChallengeInput) -> NChallengeOutput:
        ...
        log.trace("Raw solver response: %r", response)
...
class DenoSolver(Solver):
    def solve(self, challenge: NChallengeInput) -> NChallengeOutput | None:
        log.trace("Solving n-challenge token %r via Deno", challenge.token)
        try:
            ...
            if "The system cannot find the file specified" in str(exc):
                raise PluginError(
                    "Deno not found. Please install Deno from https://deno.land/manual/getting_started/installation",
                ) from exc
...
class YoutubeAPI:
    ...
    def process_live_page(self, url: str) -> str:
        ...
        initial = self.get_data_from_regex(res, re_yt_initial_data, "initial data")
    ...
    def _extract_hls(self, player_responses: list[dict], player_url: str) -> list[str]:
        ...
        for response in player_responses:
            ...
            if n_matches and yt_dlp_solver is None:
                ...
                n_token = n_matches[0]
                log.trace("Solving n-challenge token: %s", n_token)
                ...
                if result and (solved := result.results.get(n_token, "")):
                    ...
                    log.trace("n-challenge solved: %s -> %s", n_token, solved)
    ...
    def get_streams_from_api(self, url, video_id) -> dict | None:
        ...
        return HLSStream.parse_variant_playlist(self.session, hls_url, name_key="pixels")
...
class YouTube(Plugin):
    ...
    def _get_streams(self):
        ...
        try:
            ...
            for hls_url in m3u8_urls:
                return HLSStream.parse_variant_playlist(self.session, hls_url)

            raise ValueError(f"No HLS URLs found for a videoId={self.match['video_id']}")

        except (PluginError, ValueError) as e:
            log.error("Extraction failed: %s", e)
            log.info("Falling back to YouTube API")
            return api.get_streams_from_api(self.url, self.match["video_id"])
